...
const exclude = new Set([...(b1.ids || []), ...(b2.ids || [])]);
...
if (overlap.length) {
  console.error("OVERLAP", overlap.map((x) => x.id));
  process.exit(1);
}

const out = {
  selected_count: selected.length,
  excluded_batch1_2: exclude.size,
  criterion:
    "score>=5, exclude batch1∪batch2 + meta + already_rich, diversify by part_id, top 50",
...
fs.writeFileSync(
  "scripts/output/aws_scenarios_enrich_batch_3.json",
  ...
  JSON.stringify(
    {
      ...
      by_part: byPartCount,
      ids: selected.map((s) => s.id),
      samples: selected.slice(0, 5).map((s) => ({
        id: s.id,
        part: s.part_id,
        score: s.score,
        e: s.enunciado,
