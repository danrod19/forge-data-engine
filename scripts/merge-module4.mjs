...
const parts = ["4.1", "4.2", "4.3", "4.4", "4.5"];
...
fs.writeFileSync(
  path.join(dataDir, "questions_module4_traditional.json"),
  JSON.stringify(out) + "\n"
);
console.log(
  "M4_TRAD",
...
fs.writeFileSync(
  path.join(dataDir, "tickets_module4.json"),
  JSON.stringify(tout) + "\n"
);
console.log(
  "M4_TICK",
...
const m2 = fs.existsSync(path.join(dataDir, "tickets_module2.json"))
  ...
  : [];
const m3 = fs.existsSync(path.join(dataDir, "tickets_module3.json"))
  ? JSON.parse(fs.readFileSync(path.join(dataDir, "tickets_module3.json"), "utf8"))
  : [];
