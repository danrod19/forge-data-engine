...
const b3ids = new Set(
  JSON.parse(
    fs.readFileSync(
      path.join(root, "scripts/output/aws_scenarios_enrich_batch_3.json"),
      "utf8"
    )
  ).ids
);
...
const before = JSON.parse(
  fs.readFileSync(
    path.join(root, "scripts/output/aws_scenarios_before_enrich_batch3.json"),
    "utf8"
  )
);
...
for (let n = 1; n <= 5; n++) {
  const p = JSON.parse(
    fs.readFileSync(
      path.join(root, `scripts/output/aws_enrich_b3_patch_${n}.json`),
      "utf8"
    )
  );
...
for (const item of patched) {
  if (prior.has(item.id)) {
    errors.push(`FATAL prior-batch id ${item.id}`);
    continue;
  }
  if (!b3ids.has(item.id)) {
    errors.push(`not in batch3 ${item.id}`);
    continue;
  }
...
const tmp = path.join(os.tmpdir(), "questions_aws_traditional_b3.json");
...
fs.writeFileSync(
  path.join(root, "scripts/output/aws_scenarios_enrich_batch3_report.json"),
